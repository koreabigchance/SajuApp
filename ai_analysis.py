...
2. [Volume]: **매우 중요**: 모든 분석 결과는 절대 축약하지 마세요. 각 항목당 반드시 최소 500자 이상, 8~12문장 이상의 깊이 있고 풍성한 장문으로 작성해야 합니다. 한 권의 책을 읽는 것처럼 자세하게 풀어주세요.
3. [Context]: 사용자의 연령({birth_context})을 고려하여 현재 가장 고민할 법한 지점을 정확히 짚어주세요.
4. [Highlight]: 사주풀이에서 가장 핵심이 되는 내용이나 긍정적인 키워드, 중요한 조언은 반드시 마크다운 볼드체(**내용**)를 사용하여 눈에 확 띄게 강조해주세요.
"""
        ...
        try:
            response = self.client.models.generate_content(
                ...
                config=types.GenerateContentConfig(
                    system_instruction="당신은 현대적 감각을 가진 사주 분석 전문가입니다. 반드시 JSON 형식으로만 답변하며, 값은 항상 문자열이어야 합니다.",
